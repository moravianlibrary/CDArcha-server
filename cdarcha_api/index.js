...
var apiOpenArchive = "api/openarchive";
...
var URL_lib = require('url'); // deprecated
var fs = require('fs');
var md5 = require('md5');
...
var Server = /** @class */ (function () {
    function Server(req, response, db) {
        ...
        this.db = db;
        var urlLog = url;
        var passwdPos = url.indexOf('password=');
        if (passwdPos !== -1) {
    ...
    Server.prototype.go = function (s) {
         ...
         **/
            ...
            var fileName = this.requrl.substr(1);
         ...
         **/
            ...
            if (this.req.method == 'POST') {
                ...
                Helpers_1.Helpers.checkApiLogin(s.db, this.query.get('login'), this.query.get('password'), this.query.get('version'), apiVersion, function (resApiLogin, user) {
                    if (resApiLogin) {
         ...
         **/
            ...
            if (this.req.method == 'POST') {
                ...
                var uploadFn = tmpFolder + '/iso/' + mediaId + '.iso';
                ...
                this.req.on('data', function (chunk) {
                    ...
                    if (chunkString.match(/Content\-Type\: image\/tif/)) {
                        ...
                        if (!writeStream && fileName_1) {
                            ...
                            testLog('opening file for writing: ' + tmpFolder + '/scan/' + boundary_1 + '-' + fileName_1);
                                        ...
                                        if (filenamePrefix9 == 'cover.tif' || filenamePrefix9 == 'toc_page_') {
                                            ...
                                            fs.readFile(filenameFull, function (err, fileData) {
                                                ...
                                                Files_1.Files.insertUnique(s, fileinfo, function (result) {
                                                    ...
                                                    var copyDest = masterCopyDir + '/' + (mediaDoc ? mediaDoc._id : archiveDoc._id) + '-' + filename;
         ...
         **/
            ...
            Helpers_1.Helpers.checkApiLogin(s.db, this.query.get('login'), this.query.get('password'), this.query.get('version'), apiVersion, function (resApiLogin) {
                if (resApiLogin) {
                    s.response.writeHead(resApiLogin);
                    s.response.end();
                    return;
         ...
         **/
            ...
            var op = this.requrl.indexOf(apiOpenArchive) > 0 ? 'open' : 'close';
            var opCode_1 = op == 'close' ? 1 : 0;
            testLog("41", "[ API " + op == 'close' ? 'CLOSE' : 'OPEN' + "ARCHIVE ]");
            var id_1 = this.query.get('id');
            if (!id_1) {
                s.response.writeHead(404);
            ...
            if (!this.query.get('multi')) {
                ...
                this.query.forEach(function (queryValue, queryKey) {
                    switch (queryKey) {
                        case 'isbn':
                        case 'ean':
                            multi[queryKey] = this.query.get('isbn');
                            break;
                        case 'nbn':
                            multi[queryKey] = this.query.get('nbn');
                            break;
                        case 'issn':
                            multi[queryKey] = this.query.get('issn');
                            break;
                        case 'ismn':
                            multi[queryKey] = this.query.get('ismn');
                            break;
                        case 'oclc':
                            multi[queryKey] = this.query.get('oclc');
                            break;
                        case 'uuid':
                            multi[queryKey] = this.query.get('uuid');
                            break;
                        case 'sysno':
                            multi[queryKey] = this.query.get('sysno');
                            break;
                        case 'sigla':
                            multi[queryKey] = this.query.get('sigla');
                            break;
                    }
                });
                multis.push(multi);
