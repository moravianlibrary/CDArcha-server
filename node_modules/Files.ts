...
export class Files {
    ...
    static insert(s: any, files: Filesdata, callback) {
        ...
        Object.keys(files).forEach(function(key) {
            if (key!='archive' && key!='media' && key!='fileName' && key!='fileType' && key!='checkSum' && key!='fileSize')
            {
                delete files[key];
            }
        });
        ...
        s.db.collection(filesCollection).insert(files, { w: 1 }, function(err, result) {
            if (err) {
                console.log(err);
                callback(undefined);
            } else {
