// ====================== CONFIG.TS ===================
// url na backend
var urlMain = "195.113.155.152"; //server
// url frontendu / spolecne url vsech frontnendu
var urlReplace = "195.113.155.152"; //server
// IP adresa backendu - udsud je mozne prijimat ridici prikazy
var ipBackend = "195.113.155.152"; //server
var urlPart = "/";
...
var Server = (function () {
    ...
    Server.prototype.go = function (s) {
        ...
        if (etags[this.etag] !== undefined) {
                        ...
                        if (!media) {
                            Bibinfo.insert(s, bibinfo, function (res) {
                                if (!res) {
                                    ...
                                    if (metaxml.length)
                                        fileSystem.writeFile(storageFolder + '/meta/' + res.ops[0]._id + '.xml', metaxml);
                                    if (marcxml.length)
                                        fileSystem.writeFile(storageFolder + '/marcxml/' + res.ops[0]._id + '.xml', marcxml);
                                    if (mods.length)
                                        fileSystem.writeFile(storageFolder + '/mods/' + res.ops[0]._id + '.xml', mods);
                            ...
                            if (metaxml.length)
                                fileSystem.writeFile(storageFolder + '/meta/' + media._id + '.xml', metaxml);
                            if (marcxml.length)
                                fileSystem.writeFile(storageFolder + '/marcxml/' + media._id + '.xml', marcxml);
                            if (mods.length)
                                fileSystem.writeFile(storageFolder + '/mods/' + media._id + '.xml', mods);
                        ...
                        if (item) {
                            ...
                            if (success) {
                                ...
                                s.db.collection(filesCollection).insert(mediaFile, { w: 1 }, function (err, result) {
                                    var mediaFileId = result.ops[0]._id;
            ...
            var sigla = Permissions.ipValid(this.xreferer, this.remoteIP, this.query.sigla);
