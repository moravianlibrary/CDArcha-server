...
class Server {
    ...
    go(s: Server) {
                ...
                this.req.on('end', function() {
                    ...
                    s.db.collection(filesCollection).findOne({ "checkSum": checksum }, function(err, item) {
                        ...
                        if (item) {
                            ...
                            fileSystem.rename(tmpFolder + '/iso/' + mediaId + '.iso', storageFolder + '/iso/' + item._id + '.' + fileType);
                            ...
                            if (success) {
                                ...
                                s.db.collection(filesCollection).insert(mediaFile, { w: 1 }, function(err, result) {
                                    ...
                                    fileSystem.rename(tmpFolder + '/iso/' + mediaId + '.iso', storageFolder + '/iso/' + mediaFileId + '.' + fileType);
                                });
         ...
         **/
            ...
            this.db.collection(filesCollection).findOne({ 'quickId': this.query.quickid }, function(err, item) {
                if (err) {
                    s.response.writeHead(404);
                    s.response.end(JSON.stringify({ "status": "error", "msg": err }));
                    return;
